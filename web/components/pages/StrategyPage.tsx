...
export function StrategyPage() {
  ...
  return (
    <>
      ...
      <div className="grid-2">
        <div className="card">
          ...
          <div className="card-body">
            <Criterion num="1" label="Price > 20-day SMA" desc="Established short trend"/>
            <Criterion num="2" label="Price > 50-day SMA" desc="Confirms longer uptrend"/>
            <Criterion num="3" label="Price > 5-day high (close)" desc="Breakout trigger"/>
            <Criterion num="4" label="Volume ≥ 1.1× 20-day avg" desc="Volume confirmation"/>
            <Criterion num="5" label="RSI(14) ∈ [50, 75]" desc="Momentum without overbought"/>
            <Criterion num="6" label="No existing position" desc="One symbol, one position"/>
          </div>
        </div>
        <div className="card">
          <div className="card-head"><h3><Icon name="diamond" size={12}/> Exit Ladder (first triggered wins)</h3></div>
          <div className="card-body">
            <ExitRule prio="1" label="Hard stop" val="−2% from entry" kind="neg"/>
            <ExitRule prio="2" label="Trailing stop" val="−3% from high watermark" kind="warn"/>
            <ExitRule prio="3" label="Take profit" val="+5% from entry" kind="pos"/>
            <ExitRule prio="4" label="Trend break" val="Close below 20-day SMA" kind="muted"/>
            <ExitRule prio="5" label="EOD close" val="16:10 ET — flatten all" kind="muted"/>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        <div className="card-head"><h3><Icon name="eye" size={12}/> Approved Universe</h3><span className="text-tertiary mono" style={{fontSize:10}}>10 symbols · no exceptions</span></div>
...
function Criterion({ num, label, desc }: { num: string; label: string; desc: string }) {
  return (
    <div style={{display:'grid',gridTemplateColumns:'22px 1fr',gap:10,alignItems:'start',padding:'8px 0',borderBottom:'1px solid var(--border-subtle)'}}>
      ...
      <div><div style={{fontSize:11,fontWeight:500}}>{label}</div><div className="text-tertiary" style={{fontSize:10,marginTop:2}}>{desc}</div></div>
    </div>
  );
}

function ExitRule({ prio, label, val, kind }: { prio: string; label: string; val: string; kind: string }) {
  return (
    <div style={{display:'grid',gridTemplateColumns:'22px 1fr auto',gap:10,alignItems:'center',padding:'8px 0',borderBottom:'1px solid var(--border-subtle)'}}>
      ...
      <div><div style={{fontSize:11,fontWeight:500}}>{label}</div><div className="text-tertiary mono" style={{fontSize:10,marginTop:2}}>{val}</div></div>
